  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Maseru Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Maseru Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest contractor-registration-fee-mismatch-is-held-and-unoverridable
  (testing "declared :seeking-contractor-registration? true but claimed fee doesn't match gov.ls's own category-fee schedule -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:contractor-registration-fee-mismatch} (-> (store/ledger db) last :basis)))
